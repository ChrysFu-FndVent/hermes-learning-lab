...
1. 用户从 Lesson 00 建立 Setup 和 Doctor 基线。
2. 课前诊断暴露已有理解，不影响继续学习。
3. 用户观察四段 Agent 轨迹并完成四步概念讲解。
4. 实验区明确真实任务、输入、步骤和成功标准。
5. 浏览器课后检查给出成功或纠错反馈。
6. 通过后记录完成状态；真实实验仍需用户在隔离环境按成功标准验收。
7. 阶段完成后进入下一层；毕业项目综合安全、评测和恢复。

## 技术架构

```mermaid
flowchart LR
    A[官方文档 / 模型卡 / AI 课程参考] --> B[12 课结构化内容]
    B --> C[React 学习引擎]
    C --> D[诊断与课后判定]
    D --> E[localStorage v2]
    C --> F[实验手册与资料视图]
